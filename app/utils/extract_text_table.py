...
import numpy as np
from app.utils.table_process import *
...
def extract_and_process_table_from_image(image: Image, threshold: float = 0.7) -> str:
    cropped_tables = crop_tables_from_image(image)
    result_text = []

    for table in cropped_tables:
        w, h = table.size

        # Xử lý ảnh bảng với Table Transformer
        inputs = str_proc(
            images=table,
            return_tensors="pt",
            do_resize=False
        ...
        # Phân loại phần tử thành hàng, cột và bảng
        ...
        # Gắn kết các văn bản OCR vào các ô
        ...
        # Xây dựng bảng dưới dạng văn bản
        result_table = []
        for (row_idx, col_idx), cell in cells.items():
            # Sắp xếp các văn bản trong ô theo thứ tự vị trí
        ...
        # Chuyển kết quả bảng thành văn bản dễ đọc
        rows_data = {}
        ...
        result_text.append(' '.join(table_text))
